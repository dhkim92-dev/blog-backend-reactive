package kr.dhkim92.blog_reactive.interfaces.auth

import io.swagger.v3.oas.annotations.Operation
import io.swagger.v3.oas.annotations.tags.Tag
import kr.dhkim92.blog_reactive.application.auth.AuthServiceFacade
...
class AuthController(
    ...
    @Envelope(status = CREATED, message = "Login successful", code = "AT001")
        ...
        return authServiceFacade.loginByEmailPassword(request.toCommand())
            .flatMap { loginResult ->
                CookieUtil.setCookie(REFRESH_TOKEN_COOKIE_NAME, loginResult.refreshToken, REFRESH_TOKEN_EXPIRE_SECONDS)
                    .then(Mono.just(LoginResponse.from(loginResult)))
            }
    }
